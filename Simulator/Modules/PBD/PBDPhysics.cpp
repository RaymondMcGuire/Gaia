...
void GAIA::PBDPhysics::loadRunningparameters(std::string inModelInputFile,
                                             std::string inParameterFile,
  ...
  dt = physicsAllParams.physicsParams.timeStep /
       physicsAllParams.physicsParams.numSubsteps;

  // make parameters step invariant
  for (size_t iMesh = 0; iMesh < objectParamsList.objectParams.size();
       iMesh++) {
    objectParamsList.getObjectParam(iMesh).exponentialVelDamping = std::pow(
        objectParamsList.getObjectParam(iMesh).exponentialVelDamping, dt);
    objectParamsList.getObjectParam(iMesh).constantVelDamping =
        objectParamsList.getObjectParam(iMesh).constantVelDamping * dt;
  }
}

void GAIA::PBDPhysics::saveExperimentParameters(
    const std::string &paramsOutOutPath) {
...
void GAIA::PBDPhysics::syncAllToInvertedSignOnly(bool sync) {
  ...
  //	}
  // }
  // std::cout << "num inverted verts after solving: " << numInvertedVerts <<
  // std::endl;
}
...
bool GAIA::PBDPhysics::initializeGPU() {
  ...
  for (int iMesh = 0; iMesh < objectParamsList.objectParams.size(); ++iMesh) {
    ...
    MF::IO::FileParts fp =
        MF::IO::fileparts(objectParamsList.objectParams[iMesh]->path);
    ...
    if (loadSucceed) {
      ...
      case NeoHookean: {
        PBDTetMeshNeoHookean::SharedPtr pTetMeshNeoHookean =
            std::make_shared<PBDTetMeshNeoHookean>();
        tMeshes[iMesh] = pTetMeshNeoHookean;
        pTetMeshNeoHookean->initialize(objectParamsList.objectParams[iMesh],
                                       pTM_MF, this);
        GPUTMeshes[iMesh] = pTetMeshNeoHookean->getTetMeshGPU();
        break;
      }
      case MassSpring: {
        PBDTetMeshMassSpring::SharedPtr pTetMeshMassSpring =
            std::make_shared<PBDTetMeshMassSpring>();
        tMeshes[iMesh] = pTetMeshMassSpring;
        pTetMeshMassSpring->initialize(objectParamsList.objectParams[iMesh],
                                       pTM_MF, this);
      ...
      std::cout << "[Error] Fail to load: "
                << objectParamsList.objectParams[iMesh]->path << std::endl;
      exit(-1);
    }
  }
  ...
  pDCD = std::make_shared<DiscreteCollisionDetector>(
      physicsAllParams.collisionParams);
  pCCD = std::make_shared<ContinuousCollisionDetector>(
      physicsAllParams.collisionParams);
  ...
  // {
  // 	pVolCD =
  // std::make_shared<VolumetricCollisionDetector>(physicsAllParams.collisionParams.volCollisionParams);
...
void GAIA::PBDPhysics::simulateGPU() {
  ...
  while (frameId < physicsAllParams.physicsParams.numFrames) {
    ...
    std::cout << "----------------------------------------------------\n"
              << "Frame " << frameId + 1 << " completed, Time consumption: "
              << timeStatistics.timeCsmpFrame
              << "\n----------------------------------------------------"
              << std::endl;
...
void GAIA::PBDPhysics::simulateGPU_debugOnCPU() {
  ...
  for (size_t iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    switch (objectParamsList.objectParams[iMesh]->materialType) {
    case NeoHookean: {
      gpuTMeshesOnCPU.push_back(
          &(std::static_pointer_cast<PBDTetMeshNeoHookean>(tMeshes[iMesh])
                ->tetMeshGPU_forCPU));
      break;
    }
    case MassSpring:
      gpuTMeshesOnCPU.push_back(
          &(std::static_pointer_cast<PBDTetMeshMassSpring>(tMeshes[iMesh])
                ->tetMeshGPU_forCPU));
      break;
    default:
      break;
...
void GAIA::PBDPhysics::runStepGPU() {
  ...
  for (substep = 0; substep < physicsParams().numSubsteps; substep++) {
    ...
    // do the initial detection to categorize the vertices into 2 types:
    // penetration free ones and penetrated ones
...
void GAIA::PBDPhysics::runStepGPU_debugOnCPU(
    std::vector<PBDTetMeshFEMGPU *> &gpuTMeshesOnCPU) {
  timeStatistics.setToZero();
  TICK(timeCsmpAllSubSteps);
  ...
  for (substep = 0; substep < physicsParams().numSubsteps; substep++) {
    ...
    for (iIter = 0; iIter < physicsParams().iterations; iIter++) {
      ...
      if ((collisionDetectionCounter >=
               physicsParams().collisionDetectionSubSteps ||
           collisionDetectionCounter == 0) &&
          ((!physicsParams().doCollDetectionOnlyForFirstIteration) ||
           (iIter == 0))) {
        ...
        bool rebuildCCDBVH =
            (substep == 0) && !(frameId % physicsParams().ccdBVHRebuildSteps);
        ...
        bool rebuildDCDBVH =
            (substep == 0) &&
            !(frameId % physicsParams().dcdTetMeshSceneBVHRebuildSteps);
        TICK(timeCsmpUpdatingCollisionInfoDCD);
...
void GAIA::PBDPhysics::materialBoundarySolveGPU() {
  ...
  while (!allDone) {
    allDone = true;
    for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
      if (iParallelizationGroup <
          tMeshes[iMesh]->numAllParallelizationGroups()) {
        // printf("---------------------------\nparallelizationGroup:\n",
        // iParallelizationGroup);
        tMeshes[iMesh]->solveMaterialConstraintGPU_ParallelizationGroup(
            iParallelizationGroup, physicsParams().numThreadsMaterialSolve,
            cudaStreams[iMesh]);
        allDone = false;
      }
    }
  ...
  // for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++)
  //{
  //	tMeshes[iMesh]->solveMaterialGPUAggregated(physicsParams().numThreadsMaterialSolve,
  // cudaStreams[iMesh]);
  // }
...
void GAIA::PBDPhysics::materialBoundarySolveGPU_debugOnCPU(
    std::vector<PBDTetMeshFEMGPU *> &gpuTMeshesOnCPU) {
  for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    tMeshes[iMesh]->solveMaterialConstraintGPU_debugOnCPU(
        gpuTMeshesOnCPU[iMesh]);
  }
...
void GAIA::PBDPhysics::InversionSolveGPU() {
  ...
  for (size_t iSolve = 0; iSolve < physicsParams().inversionSolveIterations;
       iSolve++) {
    bool allDone = false;
    int iParallelizationGroup = 0;
    while (!allDone) {
      ...
      for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
        ...
        if (iParallelizationGroup < pTM->numTetParallelizationGroups()) {
          // printf("---------------------------\nparallelizationGroup:\n",
          // iParallelizationGroup);
          solveInversionConstraintGPU(
              pTM->getTetParallelizationGroupBufferGPU(iParallelizationGroup),
              pTM->getTetParallelizationGroupSize(iParallelizationGroup),
              physicsParams().numThreadsInversionSolve, cudaStreams[iMesh],
              GPUTMeshes[iMesh],
              physicsParams().inversionSolveConstraintMultiplier);
          allDone = false;
        }
      }
    ...
    //	PBDTetMeshFEM* pTM = tMeshes[iMesh].get();
    //	for (int iGroup = 0; iGroup < pTM->numTetParallelizationGroups();
    // iGroup++)
    //	{
    //		solveInversionConstraintGPU(pTM->getTetParallelizationGroupBufferGPU(iGroup),
    // pTM->getTetParallelizationGroupSize(iGroup),
    //			physicsParams().numThreadsInversionSolve,
    // cudaStreams[iMesh], GPUTMeshes[iMesh],
    // physicsParams().inversionSolveConstraintMultiplier);
    //	}
    //
    // });
  ...
  for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    evaluateInversionGPU(physicsParams().numThreadsInversionSolve,
                         cudaStreams[iMesh], tMeshes[iMesh]->numTets(),
                         GPUTMeshes[iMesh]);
  }
}
...
void GAIA::PBDPhysics::BoundarySolve() {
  ...
  for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    ...
    if (physicsParams().usePlaneGround) {
      solveBoxBoundaryConstraint(
          cudaStreams[iMesh], tMeshes[iMesh]->numVertices(),
          physicsParams().numThreadsBoundarySolve, GPUTMeshes[iMesh],
          worldBounds(0, 0), worldBounds(0, 1), worldBounds(1, 0),
          worldBounds(1, 1), worldBounds(2, 0), worldBounds(2, 1),
          physicsParams().boundaryFrictionStatic,
          physicsParams().boundaryFrictionDynamic);
    }
  }
}

void GAIA::PBDPhysics::BoundarySolveSolveGPU_debugOnCPU(
    std::vector<PBDTetMeshFEMGPU *> &gpuTMeshesOnCPU) {
  auto &worldBounds = physicsParams().worldBounds;
  for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    // tMeshes[iMesh]->solveMaterialConstraintGPU_ParallelizationGroup(iParallelizationGroup,
    // physicsParams().numThreadsMaterialSolve, cudaStreams[iMesh]);
    if (physicsParams().usePlaneGround) {
      solveBoxBoundaryConstraintOnCPU(
          gpuTMeshesOnCPU[iMesh], worldBounds(0, 0), worldBounds(0, 1),
          worldBounds(1, 0), worldBounds(1, 1), worldBounds(2, 0),
          worldBounds(2, 1), physicsParams().boundaryFrictionStatic,
          physicsParams().boundaryFrictionDynamic);
    }
  }
}
...
void GAIA::PBDPhysics::initialCollisionsDetection() {
  ...
  // all of the vertices should be processed by DCD, thus we put all of them in
  // initialPenetratedVertIds
  if (!collisionParams().allowCCD) {
    for (size_t iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
      PBDTetMeshFEM *pTM = tMeshes[iMesh].get();
...
void GAIA::PBDPhysics::updateDCD_CPU(bool rebuild) {
  ...
  RTCBuildQuality surfaceSceneQuality =
      !(frameId % physicsParams().dcdSurfaceSceneBVHRebuildSteps) &&
              (substep == 0)
  ...
  if (!collisionParams().allowCCD) {
    for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
      ...
      cpu_parallel_for(0, pTM->surfaceVIds().size(), [&](int iSurfaceV) {
        ...
        if (pTM->DCDEnabled(surfaceVId)) {
          pDCD->vertexCollisionDetection(surfaceVId, iMesh,
                                         &dcdResults[iSurfaceV]);
        }
      });
      ...
      for (int iSurfaceV = 0; iSurfaceV < pTM->surfaceVIds().size();
           iSurfaceV++) {
        if (dcdResults[iSurfaceV].numIntersections()) {
      ...
      cpu_parallel_for(0, dcdResultsPositiveVId.size(), [&](int iIntersection) {
        ...
        pDCD->closestPointQuery(
            &dcdResults[dcdResultsPositiveVId[iIntersection]],
            &closestPtResult);
      });
      TOCK_STRUCT(timeStatistics, timeCsmpShortestPathSearchDCD);

      for (int iIntersection = 0; iIntersection < dcdResultsPositiveVId.size();
           iIntersection++) {
        positiveCollisionDetectionResults.push_back(
            dcdResults[dcdResultsPositiveVId[iIntersection]]);
      }
    }
  } else {
    for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
      ...
      TetMeshFEM *pTM = tMeshes[iMesh].get();
      std::vector<CollisionDetectionResult> dcdResults(
          initialPenetratedVertIds[iMesh].size());

      TICK(timeCsmpColDetectDCD);
      cpu_parallel_for(0, dcdResults.size(), [&](int iSurfaceV) {
        int32_t surfaceVId =
            pTM->surfaceVIds()(initialPenetratedVertIds[iMesh][iSurfaceV]);
        if (pTM->DCDEnabled(surfaceVId)) {
          pDCD->vertexCollisionDetection(surfaceVId, iMesh,
                                         &dcdResults[iSurfaceV]);
        }
  ...
  for (int iCol = numActiveCCD; iCol < positiveCollisionDetectionResults.size();
       iCol++) {
    collisionStatistics.numOfBVHQuerysEachStep.back().push_back(
        positiveCollisionDetectionResults[iCol].numberOfBVHQuery);
    collisionStatistics.numOfTetTraversal.back().push_back(
        positiveCollisionDetectionResults[iCol].numberOfTetTraversal);
    collisionStatistics.numTetsTraversed.back().push_back(
        positiveCollisionDetectionResults[iCol].numberOfTetsTraversed);
  }
#endif // DO_COLLISION_STATISTICS

  // std::cout << "Number of collisions detected by DCD:" << numActiveDCD <<
  // std::endl;
}
...
void GAIA::PBDPhysics::updateCCD_CPU(bool rebuild) {
  ...
  for (int iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    ...
    cpu_parallel_for(
        0, initialPenetrationFreeVertIds[iMesh].size(), [&](int iSurfaceV) {
          int32_t surfaceVIdSurfaceMesh =
              initialPenetrationFreeVertIds[iMesh][iSurfaceV];
          int32_t surfaceVId = pTM->surfaceVIds()(surfaceVIdSurfaceMesh);
          if (pTM->CCDEnabled(surfaceVId)) {
            pCCD->vertexContinuousCollisionDetection(
                surfaceVId, iMesh,
                &ccdResultsAll[iMesh][surfaceVIdSurfaceMesh]);
          }
        });

    for (int iSurfaceV = 0;
         iSurfaceV < initialPenetrationFreeVertIds[iMesh].size(); iSurfaceV++) {
      int32_t surfaceVIdSurfaceMesh =
          initialPenetrationFreeVertIds[iMesh][iSurfaceV];
  ...
  // std::cout << "Number of collisions detected by CCD:" << numActiveCCD <<
  // std::endl;
}
...
void GAIA::PBDPhysics::updateVelocities() {
  for (size_t iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    tMeshes[iMesh]->mVelocity =
        (tMeshes[iMesh]->mVertPos - tMeshes[iMesh]->mVertPrevPos) * (1.0f / dt);

    VecDynamic velMags = tMeshes[iMesh]->mVelocity.colwise().norm();
    FloatingType maxVelocityMagnitude =
        getObjectParam(iMesh).maxVelocityMagnitude;
    ...
    cpu_parallel_for(0, tMeshes[iMesh]->numVertices(), [&](int iVert) {
      ...
      if (getObjectParam(iMesh).hasNoGravZone && maxVelocityMagnitude > 0 &&
          tMeshes[iMesh]->mVertPos(GRAVITY_AXIS, iVert) >
              getObjectParam(iMesh).noGravZoneThreshold)
      ...
      // apply the maximum velocity constraint
      {
        ...
        vMagNew = vMag * getObjectParam(iMesh).exponentialVelDamping -
                  getObjectParam(iMesh).constantVelDamping;
...
void GAIA::PBDPhysics::solveCollisionConstraintsCPU() {
  ...
  for (int iPenetratedV = 0; iPenetratedV < numCollisions; iPenetratedV++) {
    ...
    CollisionDetectionResult &colResult =
        positiveCollisionDetectionResults[iPenetratedV];

    for (int iIntersection = 0; iIntersection < colResult.numIntersections();
         iIntersection++) {
      if (colResult.collidingPts[iIntersection].shortestPathFound) {
        solveCollisionConstraintsOneVertCPU(curVertID, curMeshID, colResult,
                                            iIntersection);
      }
    }
  }
...
void GAIA::PBDPhysics::solveCollisionConstraintsOneVertCPU(
    int32_t curVertID, int32_t curMeshID, CollisionDetectionResult &colResult,
    int32_t iIntersection) {
  int collidedMeshID = colResult.collidingPts[iIntersection].intersectedMeshId;
  int closestFaceId =
      colResult.collidingPts[iIntersection].closestSurfaceFaceId;
  ...
  Vec3I closestFaceVIds =
      pIntersectedTM->surfaceFacesTetMeshVIds().col(closestFaceId);

  if (
      //(iIter == 0 || !physicsParams().doCollDetectionOnlyForFirstIteration)
      //&& !colResult.fromCCD
      // collision solution may influence the shape of the mesh thus the closest
      // point information maybe inaccurate even for the first iteration
  ...
  // the closest point is still valid
  {
    ...
    closestSurfacePoint =
        colResult.collidingPts[iIntersection].closestSurfacePt;
    barycentrics =
        colResult.collidingPts[iIntersection].closestSurfacePtBarycentrics;
  } else
  // recompute the closest point
  {
    embree::Vec3fa a = loadVertexPos(pIntersectedTM,
                                     pIntersectedTM->surfaceFacesTetMeshVIds()(
                                         0, closestFaceId)),
                   b = loadVertexPos(pIntersectedTM,
                                     pIntersectedTM->surfaceFacesTetMeshVIds()(
                                         1, closestFaceId)),
                   c = loadVertexPos(pIntersectedTM,
                                     pIntersectedTM->surfaceFacesTetMeshVIds()(
                                         2, closestFaceId)),
                   p = loadVertexPos(pCurTM, colResult.idVQuery);

    embree::Vec3fa bary;
    embree::Vec3fa closestSurfacePoint_ =
        closestPointTriangle(p, a, b, c, bary, pointType);

    closestSurfacePoint << closestSurfacePoint_.x, closestSurfacePoint_.y,
        closestSurfacePoint_.z;
  ...
  case GAIA::ClosestPointOnTriangleType::AtA:
    surfaceVIdTMeshIndex =
        pIntersectedTM->surfaceFacesTetMeshVIds()(0, closestFaceId);
    surfaceVIdSurfaceIndex =
        pIntersectedTM->surfaceFacesSurfaceMeshVIds()(0, closestFaceId);
  ...
  case GAIA::ClosestPointOnTriangleType::AtB:
    ...
    surfaceVIdSurfaceIndex =
        pIntersectedTM->surfaceFacesSurfaceMeshVIds()(1, closestFaceId);
    pIntersectedTM->computeVertexNormal(surfaceVIdSurfaceIndex, normal);
    break;
  case GAIA::ClosestPointOnTriangleType::AtC:
    ...
    surfaceVIdSurfaceIndex =
        pIntersectedTM->surfaceFacesSurfaceMeshVIds()(2, closestFaceId);
    pIntersectedTM->computeVertexNormal(surfaceVIdSurfaceIndex, normal);
    break;
  ...
  Vec12 grads;
  grads << dC0[0], dC0[1], dC0[2], dC1[0], dC1[1], dC1[2], dC2[0], dC2[1],
      dC2[2], normal[0], normal[1], normal[2];

  applyCollisionConstraintsCPU(closestFaceVIds, curVertID, curMeshID,
                               collidedMeshID, C, penetrationDepth, dt,
                               barycentrics, grads);
}

void GAIA::PBDPhysics::applyCollisionConstraintsCPU(
    const Vec3I &closestFaceVIds, int32_t curVertId, int32_t curMeshID,
    int32_t collidedMeshID, FloatingType C, FloatingType penetrationDepth,
    FloatingType dt, Vec3 &barycentric, const Vec12 &gradients) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    w += gradients.block<3, 1>(i * 3, 0).squaredNorm() *
         pIntersectedTM->vertexInvMass(
             vId); // ((*softBodies)[mID]->invMass[id]);
  }

  w += gradients.block<3, 1>(9, 0).squaredNorm() *
       pCurTM->vertexInvMass(curVertId);
  ...
  for (int i = 0; i < 3; i++) {
    int vId = closestFaceVIds[i];
    Vec3 correction = dlambda * pIntersectedTM->vertexInvMass(vId) *
                      gradients.block<3, 1>(i * 3, 0);
    pIntersectedTM->vertex(vId) += correction;
    velocities[i] =
        pIntersectedTM->vertex(vId) - pIntersectedTM->vertexPrevPos(vId);
  }

  Vec3 correction =
      dlambda * pCurTM->vertexInvMass(curVertId) * gradients.block<3, 1>(9, 0);
  ...
  Vec3 closestPVelocity = velocities[0] * barycentric[0] +
                          velocities[1] * barycentric[1] +
                          velocities[2] * barycentric[2];
  ...
  FloatingType frictionRatioDynamic =
      objectParamsList.getObjectParam(curMeshID).friction_dynamic;
  FloatingType frictionRatioStatic =
      objectParamsList.getObjectParam(curMeshID).friction_static;
  ...
  Vec3 diffV = diff * pCurTM->vertexInvMass(curVertId) /
               (pCurTM->vertexInvMass(curVertId) +
  ...
  // FloatingType d = pIntersectedTM->vertexInvMass(closestFaceVIds[0]) *
  // pow(barycentric[0],2)
  //	+ pIntersectedTM->vertexInvMass(closestFaceVIds[1]) *
  // pow(barycentric[1], 2)
  //	+ pIntersectedTM->vertexInvMass(closestFaceVIds[2]) *
  ...
  // pIntersectedTM->vertexInvMass(closestFaceVIds[0]) * barycentric[0] / d;
  // pIntersectedTM->vertex(closestFaceVIds[1]) += diffTriangle *
  // pIntersectedTM->vertexInvMass(closestFaceVIds[1]) * barycentric[1] / d;
  // pIntersectedTM->vertex(closestFaceVIds[2]) += diffTriangle *
  // pIntersectedTM->vertexInvMass(closestFaceVIds[2]) * barycentric[2] / d;
  ...
  // for (int i = 0; i < 3; i++) {
  //	int vId = closestFaceVIds[i];
  //	Vec3 diffPerVert = dlambda * pIntersectedTM->vertexInvMass(vId) *
  // gradients.block<3, 1>(i * 3, 0); 	pIntersectedTM->vertex(vId) +=
  // correction; 	velocities[i] = pIntersectedTM->vertex(vId) -
  // pIntersectedTM->vertexPrevPos(vId);
  // }

  // Vec3 correction = dlambda * pCurTM->vertexInvMass(curVertId) *
  // gradients.block<3, 1>(9, 0); pCurTM->vertex(curVertId) += correction;
}
...
void GAIA::PBDPhysics::writeOutputs(std::string outFolder, int frameId) {
  ...
  std::string outFile = outFolder + "/A" + outNumber + "." +
                        physicsAllParams.physicsParams.outputExt;
  ...
  for (size_t iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    basetetMeshes.push_back(
        std::static_pointer_cast<TetMeshFEM>(tMeshes[iMesh]));
  }
  if (physicsAllParams.physicsParams.outputExt == "bin") {
    ...
    //  template
    {
      ...
      std::string templateMeshesName =
          outFolder + "/TemplateMesh/TemplateMesh.ply";
      writeAllToPLY(templateMeshesName.c_str(), basetetMeshes, false, true);
    }
    writeAllToBinary(outFile.c_str(), basetetMeshes);

    if (!(frameId %
          physicsAllParams.physicsParams.binaryModeVisualizationSteps)) {
      std::string outFileVis = outFolder + "/A" + outNumber + ".ply";

      writeAllToPLY(outFileVis.c_str(), basetetMeshes,
                    physicsAllParams.physicsParams.saveAllModelsTogether,
                    physicsAllParams.physicsParams.saveAllModelsTogether);
    }
  } else if (physicsAllParams.physicsParams.outputExt == "ply") {

    writeAllToPLY(outFile.c_str(), basetetMeshes,
                  physicsAllParams.physicsParams.saveAllModelsTogether);
  } else if (physicsAllParams.physicsParams.outputExt == "obj") {
    // writeAllToObj(outFile.c_str(), getSoftBodies(),
    // physicsAllParams.physicsParams.saveAllModelsTogether);
  } else {
    std::cout << "[Error] Unsupported output EXT name: "
              << physicsAllParams.physicsParams.outputExt << std::endl;
    return;
  }
  ...
  if (physicsAllParams.physicsParams.outputStatistics) {
    // pSoftbodyManager->statistics.writeToJsonFile(outFolder +
    // "/Statistics.json");
    timeStatistics.writeToJsonFile(tetMeshOutStatistics + "/A" + outNumber +
                                   ".json");
    if (frameId % physicsAllParams.physicsParams.outputRecoveryStateStep == 0) {
#ifdef DO_COLLISION_STATISTICS
      collisionStatistics.writeToJsonFile(tetMeshOutStatistics +
                                          "/CollisionStatisticsAll.json");
#endif // DO_COLLISION_STATISTICS
    }
  }

  if (physicsAllParams.physicsParams.outputRecoveryState &&
      !(frameId % physicsAllParams.physicsParams.outputRecoveryStateStep) &&
      (frameId)) {
...
bool GAIA::PBDPhysics::initializeFromState(std::string &stateFile) {
  ...
  std::cout << "----------------------------------------------------\n"
            ...
            << "----------------------------------------------------\n";
  // state.loadFromJsonFile(stateFile);
  // state.initializeSoftBodyManager(*pSoftbodyManager,
  // pSoftbodyManager->frameId);
...
void GAIA::PBDPhysics::setUpOutputFolders(std::string outFolder) {
  ...
  std::string tetMeshOutOutPath = outFolder + "/TetMesh";
  if (physicsAllParams.physicsParams.outputVTK ||
      physicsAllParams.physicsParams.outputT) {
    MF::IO::createFolder(tetMeshOutOutPath);
  }

  if (physicsAllParams.physicsParams.saveSimulationParameters) {
    std::string paramsOutOutPath =
        outFolder + "/SimulationParameters_" + MF::Time::getCurTimeString();
...
void GAIA::PBDPhysics::applyInitialGuesses() {
  cpu_parallel_for(0, tMeshes.size(),
                   [&](int iMesh) { tMeshes[iMesh]->applyInitialGuess(); });
}
...
void GAIA::PBDPhysics::applyConstraints() {
  for (size_t iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    PBDTetMeshFEM *pTM = tMeshes[iMesh].get();
    for (size_t iFixPoint = 0;
         iFixPoint < pTM->pObjectParams->fixedPoints.size(); iFixPoint++) {
...
void GAIA::PBDPhysics::applyPostSolvingDeformers() {
  // apply deformers
  for (size_t iDeformer = 0; iDeformer < postSolvingDeformers.size();
       iDeformer++) {
    (*postSolvingDeformers[iDeformer])(this, curTime, frameId, substep, iIter,
                                       dt);
  }
}
...
void GAIA::PBDPhysics::solveInvertedTets() {
  for (size_t iMesh = 0; iMesh < tMeshes.size(); iMesh++) {
    ...
    cpu_parallel_for(0, pTM->numTets(), [&](int iTet) {
      FloatingType v = CuMatrix::tetOrientedVolume(
          pTM->mVertPos.data(), pTM->tetVIds().col(iTet).data());
      if (v <= 0) {
    ...
    // for (int iTet = 0; iTet < pTM->numTets(); iTet++)
    //{
    //	if (CuMatrix::tetOrientedVolume(pTM->mVertPos.data(),
    // pTM->tetVIds().col(iTet).data()) < 0) {
    //		++numInverted;
    //	}
    // }
    ...
    if (physicsParams().solveInvertedTets) {
      ...
      for (int i_ = 0; i_ < inversionSolveIters; i_++) {
        ...
        for (int iTet = 0; iTet < pTM->numTets(); iTet++) {
          if (pTM->tetsInvertedSign[iTet]) {
            ...
            if (CuMatrix::tetOrientedVolume(pTM->mVertPos.data(),
                                            pTM->tetVIds().col(iTet).data()) >
                0) {
              pTM->tetsInvertedSign[iTet] = false;
            }

#ifdef TET_TET_ADJACENT_LIST
            // for (int iNeiTet = 0; iNeiTet <
            // pTM->tetAllNeighborTets[iTet].size(); iNeiTet++)
            cpu_parallel_for(
                0, pTM->tetAllNeighborTets[iTet].size(), [&](int iNeiTet) {
                  int neiTetId = pTM->tetAllNeighborTets[iTet][iNeiTet];
                  if (!pTM->tetsInvertedSign[neiTetId])
                    if (CuMatrix::tetOrientedVolume(
                            pTM->mVertPos.data(),
                            pTM->tetVIds().col(neiTetId).data()) <= 0) {
      ...
      //	}
      // }
      // std::cout << "num inverted after solving: " << numInverted <<
      // std::endl;
      ...
      //		possibleInvertedTetIds.insert(pTM->tetAllNeighborTets[iTet].cbegin(),
      //(pTM->tetAllNeighborTets[iTet].cend())); 		if (
      // CuMatrix::tetOrientedVolume(pTM->mVertPos.data(),
      // pTM->tetVIds().col(iTet).data()) > 0)
      //		{
      //			pTM->tetsInvertedSign[iTet] = false;
      //		}
      ...
      //	{
      //		if (numAttempts <=
      // physicsParams().maxInversionSolveMultiplier * numInverted ||
      // isSurfaceTet)
      //		{
      //			++numAttempts;
      //			solveInversionConstraint(pTM, iTet);
      //			possibleInvertedTetIds.insert(pTM->tetAllNeighborTets[iTet].cbegin(),
      // pTM->tetAllNeighborTets[iTet].cend());
      //		}
      //		else
      //			// remove non-surface tets when numAttempts >
      // physicsParams().maxInversionSolveMultiplier
      //		{
      //			possibleInvertedTetIds.erase(pBegin);
      //		}

      //		if (numAttempts >
      // physicsParams().maxSurfaceInversionSolveMultiplier * numInverted)
      //		{
      //			break;
      //		}
      //	}
    ...
    // std::fill(pTM->verticesInvertedSign.begin(),
    // pTM->verticesInvertedSign.end(), false);
...
void GAIA::PBDPhysics::solveInversionConstraint(PBDTetMeshFEM *pTM, int iTet) {
  ...
  std::array<FloatingType, 12> grads = {
      gradC0[0], gradC0[1], gradC0[2], gradC1[0], gradC1[1], gradC1[2],
      gradC2[0], gradC2[1], gradC2[2], gradC3[0], gradC3[1], gradC3[2]};

  if (vol < 0) {
    FloatingType C =
        6 * (vol - (1.f / pTM->tetInvRestVolume(iTet)) *
                       physicsParams().inversionSolveConstraintMultiplier);
    applyToInfiniteStiffness(pTM->mVertPos.data(), pTM->vertexInvMass.data(),
                             iTet, pTM->tetVIds().col(iTet).data(), C,
                             grads.data());
  }
}
...
void GAIA::PBDPhysics::evaluateConvergence(int iStep, int iIteration) {
  ...
  for (size_t iObj = 0; iObj < tMeshes.size(); iObj++) {
    timeStatistics.meritEnergy[iStep].back().push_back(
        tMeshes[iObj]->evaluateMeritEnergy());
  }
}
...
size_t GAIA::ObjectsParamList::size() { return objectParams.size(); }
...
bool GAIA::ObjectsParamList::fromJson(nlohmann::json &objectParam) {
  ...
  for (nlohmann::json &model : modelsInfo) {
    ...
    if (materialName == "NeoHookean") {
      ...
      std::cout << "Warning!!! Material name: " << materialName
                << " not recognized! Skipping this material!\n";
...
bool GAIA::PBDPhysicsAllParameters::fromJson(
  ...
  //     collisionParams.volCollisionParams.fromJson(
  //         physicsJsonParams["VolCollisionParams"]);
  //   }
  return true;
}
...
bool GAIA::PBDPhysicsAllParameters::toJson(nlohmann::json &physicsJsonParams) {
  ...
  //   if (collisionParams.allowVolumetricCollision) {
  //     collisionParams.volCollisionParams.toJson(
  //         physicsJsonParams["VolCollisionParams"]);
  //   }
  return true;
}
