...
struct GraphColoringParams {
  ...
  GraphColoringParams()
    ...
    options.add_options()(
        "e,edgeGraph",
        "Convert edge constraints to graph, works for trimesh and tetmesh.",
        cxxopts::value<bool>(edgeGraph))(
        "t,tGraph", "Convert tet/triangle based constraints to graph.",
        cxxopts::value<bool>(tGraph))("v,vertexGraph",
                                      "Convert vertex connections to graph.",
                                      cxxopts::value<bool>(vertexGraph))(
        "b,balance", "Whether to balance the size of each color.",
        cxxopts::value<bool>(balanceGraphColoring))(
        "r,ratio", "The goal max/min ratio of the color sizes.",
        cxxopts::value<float>(goalMaxMinRatio))(
        "E,edgeEnergy",
        "Whether to added edge based bending energy to trimesh vertex graph.",
        cxxopts::value<bool>(edgeEnergy))(
        "a,algorithm",
        "The name of the coloring algorithm, can be mcs or greedy.",
...
GraphColor::SharedPtr getColoringAlgoritm(const Graph &graph,
                                          const GraphColoringParams &params) {
...
std::shared_ptr<Graph> loadMesh(const std::string &inModelInputFile,
                                const GraphColoringParams &config,
                                TMeshStaticF::SharedPtr pTM,
                                TriMeshStaticF::SharedPtr pMesh) {
  ...
  if (fp.ext == ".t") {
    ...
    std::cerr << "Error! Unrecognized input extension name: " << fp.ext
              << std::endl;
    exit(-1);
  }
  ...
  if (config.edgeGraph) {
    ...
    if (meshType == 0) {
      ...
      std::cerr
          << "Error! Edge graph for triangular mesh is not implemented yet: "
          << fp.ext << std::endl;
      exit(-1);
    }
    ...
    std::cout << "Please specify which kind of graph you want to generate "
                 "(edge/tet/vertex).\n";
  }
...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<Graph> pGraph =
      loadMesh(inModelInputFile, config, pTM, pMesh);
